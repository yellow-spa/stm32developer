...
#endif*/
#ifdef EN_USART2_RX
	 Uart2SendBuffer(data_to_send, length);
#endif
}
...
void ATKPackage_Data_Receive_Prepare(u8 data)
{
	...
	else if(state==5)
	{
		state = 0;
		RxBuffer[4+_data_cnt]=data;
		ATKPackage_Data_Receive_Anl(RxBuffer,_data_cnt+5);
	}
	else
		state = 0;
}

void ATKPackage_Data_Receive_Anl(u8 *data_buf,u8 num)
{
	u8 sum = 0,i;
	for(i=0;i<(num-1);i++)
		sum += *(data_buf+i);
